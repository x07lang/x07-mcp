...
- `x07lang-mcp@0.2.10`: the web-ui (`x07.web_ui.*`), device (`x07.device.*`), app (`x07.app.*`), and platform (`lp.*`) packs are marked maintenance mode after the 2026-06 scope cut — their backing repos (`x07-web-ui`, `x07-device-host`, `x07-platform*`) are archived and read-only on GitHub; the packs stay in the server and remain functional but receive security and compatibility fixes only.
