...
for r in "${search_roots[@]}"; do
  ...
  while IFS= read -r -d '' f; do
    ...
    if grep -Eq '"d"[[:space:]]*:' "$f"; then
      echo "ERROR: private JWK material must not be committed outside tests/fixtures: $f" >&2
      bad=1
    fi
  done < <(
    find "$r" \
      \( -type d \( -name .git -o -name .x07 -o -name target -o -name dist -o -name .agent_cache \) -prune \) -o \
      -type f -name '*.jwk.json' -print0
  )
done
...
echo "ok: no private JWK material detected"
